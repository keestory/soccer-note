    ...
    <SafeAreaView style={s.fill} edges={['top', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, gap: 12 }} keyboardShouldPersistTaps="handled">
        <Field label={t.opponentNameRequired}>
          <TextInput style={s.input} value={opponent} onChangeText={setOpponent} placeholder={t.opponentPlaceholder} placeholderTextColor="#555" />
        </Field>
        ...
        <Pressable style={s.btn} onPress={submit} disabled={saving}>
          {saving ? <ActivityIndicator color="#0a0a0a" /> : <Text style={s.btnText}>{t.createMatch}</Text>}
        </Pressable>
...
const s = StyleSheet.create({
  ...
  label: { color: theme.muted2, fontSize: 11, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 2 },
  input: { color: theme.white, fontSize: 16, paddingVertical: 4 },
  btn: { backgroundColor: theme.accent, borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginTop: 6 },
  btnText: { color: '#0a0a0a', fontWeight: '900', fontSize: 16 },
  note: { color: theme.muted2, fontSize: 12, textAlign: 'center', marginTop: 4 },
})
