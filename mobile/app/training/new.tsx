    ...
    <SafeAreaView style={s.fill} edges={['top', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, gap: 14 }} keyboardShouldPersistTaps="handled">
            ...
            {TYPES.map(({ key, label }) => {
              ...
              return (
                <Pressable key={key} onPress={() => setType(key)}
                  style={[s.typeBtn, { borderColor: active ? c : 'transparent', backgroundColor: active ? `${c}1f` : '#1a1a1a' }]}>
                  <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: c }} />
                  <Text style={{ color: active ? c : '#666', fontWeight: '700', fontSize: 12 }}>{label}</Text>
                </Pressable>
              )
            })}
          </View>
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
            {DURATIONS.map((d) => (
              <Pressable key={d} onPress={() => setDuration(d)}
                style={[s.durBtn, { backgroundColor: duration === d ? theme.accent : '#1a1a1a' }]}>
                <Text style={{ color: duration === d ? '#0a0a0a' : '#666', fontWeight: '900' }}>{t.minutesN.replace('{n}', String(d))}</Text>
              </Pressable>
            ))}
          </View>
        </View>
        ...
        <View style={s.card}>
          ...
          <TextInput style={s.input} value={location} onChangeText={setLocation} placeholder={t.locationPlaceholder} placeholderTextColor="#555" />
        </View>

        <Pressable style={s.btn} onPress={submit} disabled={saving}>
          {saving ? <ActivityIndicator color="#0a0a0a" /> : <Text style={s.btnText}>{t.save}</Text>}
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  )
...
const s = StyleSheet.create({
  ...
  label: { color: theme.muted1, fontSize: 11, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 1 },
  input: { color: theme.white, fontSize: 16, paddingVertical: 6, marginTop: 6 },
