  ...
  return (
    <SafeAreaView style={styles.fill} edges={['top', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
        <Text style={styles.brandSmall}>SOCCERNOTE</Text>
        <Text style={styles.title}>{t.newAccountTitle}</Text>
        <Text style={styles.subtitle}>{t.newAccountSubtitle}</Text>

        <Field label={t.nameLabel}>
          <TextInput style={styles.input} placeholder={t.namePlaceholder} placeholderTextColor="#555"
            value={name} onChangeText={setName} />
        </Field>
        <Field label={t.email}>
          <TextInput style={styles.input} placeholder="email@example.com" placeholderTextColor="#555"
            autoCapitalize="none" keyboardType="email-address" value={email} onChangeText={setEmail} />
        </Field>
        <Field label={t.password}>
          <TextInput style={styles.input} placeholder={t.passwordMin6} placeholderTextColor="#555"
            secureTextEntry value={password} onChangeText={setPassword} />
        </Field>

        <Pressable style={({ pressed }) => [styles.primaryBtn, pressed && { opacity: 0.85 }]}
          onPress={onSignup} disabled={loading}>
          {loading ? <ActivityIndicator color="#0a0a0a" /> : <Text style={styles.primaryText}>{t.getStarted}</Text>}
        </Pressable>

        <Pressable onPress={() => router.replace('/login')} style={styles.footerLink}>
          <Text style={styles.footerText}>
            {t.hasAccount} <Text style={{ color: theme.accent, fontWeight: '800' }}>{t.login}</Text>
          </Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  brandSmall: { color: theme.accent, fontSize: 13, fontWeight: '900', letterSpacing: 2, marginBottom: 6 },
  title: { color: theme.white, fontSize: 28, fontWeight: '900', marginBottom: 4 },
  subtitle: { color: theme.muted2, fontSize: 14, marginBottom: 24 },
  field: {
  ...
  fieldLabel: { color: theme.muted1, fontSize: 11, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 2 },
  input: { color: theme.white, fontSize: 16, paddingVertical: 4 },
  primaryBtn: { backgroundColor: theme.accent, borderRadius: 16, paddingVertical: 17, alignItems: 'center', marginTop: 8 },
