      ...
      <View style={s.header}>
        ...
        {isCoach && (
          <View style={{ flexDirection: 'row', gap: 16, marginLeft: 'auto' }}>
            <Pressable onPress={() => router.push(`/player/form?id=${id}`)} hitSlop={12}><Pencil color={theme.text} size={20} /></Pressable>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
        <View style={[s.card, { flexDirection: 'row', alignItems: 'center', gap: 16 }]}>
          <View style={[s.avatar, { backgroundColor: posColor }]}>
            {player.photo_url ? <Image source={{ uri: player.photo_url }} style={{ width: 64, height: 64, borderRadius: 16 }} />
              : <Text style={{ color: POS_TEXT[player.default_position], fontSize: 20, fontWeight: '900' }}>{player.number ?? '-'}</Text>}
          </View>
          <View style={{ flex: 1 }}>
          ...
          {stats.avg !== null && (
            <View style={s.avgBox}>
              <Star color={accentText} size={14} fill={accentText} />
          ...
          <View style={s.card}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 4 }}>
                <Text style={{ color: accentText, fontSize: 28, fontWeight: '900' }}>{overallRating(player.attributes)}</Text>
                <Text style={{ color: theme.textMute, fontSize: 10, fontWeight: '800', marginBottom: 4 }}>{t.overall}</Text>
              </View>
            </View>
            ...
            <View style={{ flexDirection: 'row', gap: 20, marginTop: 10, flexWrap: 'wrap' }}>
              ...
              {player.preferred_numbers ? (
                <View>
                  ...
                  <Text style={{ color: theme.text, fontWeight: '800', marginTop: 4 }}>{player.preferred_numbers}</Text>
                </View>
              ) : null}
            </View>
          </View>
        ...
        <View style={s.card}>
          ...
          <View style={s.statGrid}>
            <Stat label={t.statAppearances} value={stats.matchAtt} color={accentText} />
            <Stat label={t.goals} value={stats.goals} color="#22a06b" />
            ...
            <Stat label={t.statRecordedMatches} value={stats.games} color={theme.textMute} />
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
...
const makeStyles = (theme: Theme) => StyleSheet.create({
  ...
  avgNum: { color: theme.isDark ? theme.accent : theme.text, fontSize: 20, fontWeight: '900' },
  avgLabel: { color: theme.textMute, fontSize: 10 },
  ...
  bio: { color: theme.text, fontSize: 14, lineHeight: 20, marginTop: 10 },
  miniLabel: { color: theme.textMute, fontSize: 10, fontWeight: '800', textTransform: 'uppercase' },
  ...
  statBox: { width: '30%', backgroundColor: theme.card2, borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
  statValue: { fontSize: 22, fontWeight: '900' },
  statLabel: { color: theme.textMute, fontSize: 11, marginTop: 2 },
})
